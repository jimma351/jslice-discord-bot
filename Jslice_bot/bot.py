...
ALLOWED_ROLE = "FK"
DEFAULT_GUILD_ID = 1383300600367808613


def parse_discord_ids(raw_value: str):
    ids = set()
    for value in raw_value.replace(" ", ",").split(","):
        ...
        try:
            ...
            raise ValueError(
                f"Invalid Discord server ID '{value}' in ALLOWED_GUILD_IDS."
            )
    return ids


ALLOWED_GUILD_IDS = parse_discord_ids(
    os.getenv("ALLOWED_GUILD_IDS", str(DEFAULT_GUILD_ID))
)
if not ALLOWED_GUILD_IDS:
    ALLOWED_GUILD_IDS = {DEFAULT_GUILD_ID}

print(
    f"Allowed Discord servers: {', '.join(str(guild_id) for guild_id in sorted(ALLOWED_GUILD_IDS))}",
    flush=True,
)
...
SCRIPT_WORKSHEET_NAME = os.getenv("SCRIPT_WORKSHEET_NAME", "Script Changes")
...
print("Opening script changes worksheet...", flush=True)
SCRIPT_HEADERS = [
    "Script",
    "Summary",
    "What Was Done",
...
try:
    script_sheet = spreadsheet.worksheet(SCRIPT_WORKSHEET_NAME)
except gspread.WorksheetNotFound:
    script_sheet = spreadsheet.add_worksheet(
        title=SCRIPT_WORKSHEET_NAME,
        rows=1000,
        cols=len(SCRIPT_HEADERS),
    )

existing_script_headers = script_sheet.row_values(1)
if not existing_script_headers:
    script_sheet.append_row(SCRIPT_HEADERS)
elif existing_script_headers[: len(SCRIPT_HEADERS)] != SCRIPT_HEADERS:
    script_sheet.update("A1:F1", [SCRIPT_HEADERS])
...
# =========================
# SCRIPT CHANGELOG HELPERS
# =========================
def get_script_entries(script_name: str = ""):
    records = script_sheet.get_all_records()
    wanted = script_name.strip().lower()
    entries = []
    for row in records:
        name = str(row.get("Script", "")).strip()
        if not name:
            continue
...
def add_script_entry(
    ...
    script_sheet.append_row(
        [script_name, summary, changes, status, updated_by, timestamp],
        value_input_option="USER_ENTERED",
    )
    return timestamp


def unique_script_names():
    names = []
    seen = set()
    for row in get_script_entries():
        name = str(row.get("Script", "")).strip()
        key = name.lower()
...
async def setup_hook():
    """Keep one guild-scoped copy of each command and remove stale globals."""
    try:
        synced_count = 0
        for guild_id in sorted(ALLOWED_GUILD_IDS):
            ...
            print(
                f"Synced {len(synced)} slash commands to allowed server {guild_id}.",
                flush=True,
            )

        # Removing old global registrations fixes duplicate slash commands.
        bot.tree.clear_commands(guild=None)
        await bot.tree.sync()

        print(
            f"Synced {synced_count} total server-specific slash commands.",
            flush=True,
        )
    except Exception as e:
...
async def on_ready():
    ...
    for guild in list(bot.guilds):
        if guild.id not in ALLOWED_GUILD_IDS:
            print(
                f"Leaving unauthorized server: {guild.name} ({guild.id})",
                flush=True,
            )
            await guild.leave()
...
async def on_guild_join(guild: discord.Guild):
    if guild.id in ALLOWED_GUILD_IDS:
        print(f"Joined approved server: {guild.name} ({guild.id})", flush=True)
        return

    print(
        f"Rejected unauthorized server: {guild.name} ({guild.id})",
        flush=True,
    )
    await guild.leave()
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="Developer Script Log",
        value=(
            "`/script add` - Log a script summary and what was changed (FK role)\n"
            "`/script summary` - Show work logged for one script or all scripts\n"
            "`/script list` - Show every tracked script\n"
        ),
        inline=False,
    )
...
# =========================
# /script - GOOGLE SHEETS DEVELOPMENT LOG
# =========================
script_commands = app_commands.Group(
    name="script",
    description="Track server script summaries and changes",
)


@script_commands.command(name="add", description="Add a script update to Google Sheets")
@app_commands.rename(script_name="name")
@app_commands.describe(
    script_name="Script name, for example qs-multicharacter",
    summary="Short explanation of what the script does",
    changes="What you changed or completed",
    status="Status such as Done, Testing, or In Progress",
)
async def script_add(
    ...
    if not has_allowed_role(interaction.user):
        await interaction.response.send_message(
            "You do not have permission to add script updates.", ephemeral=True
        )
        return
    ...
    if not script_name or not summary or not changes:
        await interaction.response.send_message(
            "The script name, summary, and changes are required.", ephemeral=True
        )
        return
    ...
    try:
        ...
        embed = discord.Embed(
            title=f"Logged: {script_name}",
            url=SHEET_URL,
            description=f"**Summary:** {summary}\n**What was done:** {changes}",
            color=discord.Color.green(),
        )
        embed.add_field(name="Status", value=status, inline=True)
        embed.add_field(name="Updated", value=timestamp, inline=True)
        embed.set_footer(text=f"Added by {interaction.user.display_name}")
        await interaction.followup.send(embed=embed, ephemeral=True)
    except Exception as e:
        print(f"/script add error: {e}", flush=True)
        await interaction.followup.send(
            f"Could not add the script update: {e}", ephemeral=True
        )


@script_commands.command(
    name="summary",
    description="Show logged work for one script or the most recent work",
)
@app_commands.rename(script_name="name")
@app_commands.describe(
    script_name="Optional script name, for example qs-multicharacter"
)
async def script_summary(
    ...
    try:
        entries = await asyncio.to_thread(get_script_entries, script_name)
        if not entries:
            target = f" for **{script_name.strip()}**" if script_name.strip() else ""
            await interaction.followup.send(
                f"No script updates were found{target}. [Open the Google Sheet]({SHEET_URL})"
            )
            return

        shown_entries = entries[-10:]
        title = (
            f"Script Summary: {script_name.strip()}"
            if script_name.strip()
            else "Recent Script Work"
        )
        embed = discord.Embed(
            title=title,
            url=SHEET_URL,
            description=(
                f"Showing the newest {len(shown_entries)} of {len(entries)} logged update(s). "
                f"[Open the full Google Sheet]({SHEET_URL})"
            ),
            color=discord.Color.blue(),
        )

        for row in reversed(shown_entries):
            name = str(row.get("Script", "Unknown Script")).strip()
            status = str(row.get("Status", "Unknown")).strip()
            summary = str(row.get("Summary", "")).strip()
            changes = str(row.get("What Was Done", "")).strip()
            updated_by = str(row.get("Updated By", "Unknown")).strip()
            updated_at = str(row.get("Updated At", "Unknown")).strip()
            value = (
                f"**Summary:** {summary}\n"
                f"**What was done:** {changes}\n"
                f"**By:** {updated_by} • **When:** {updated_at}"
            )
            embed.add_field(
                name=f"{name} • {status}"[:256],
                value=value[:1024],
                inline=False,
            )
        ...
        print(f"/script summary error: {e}", flush=True)
        await interaction.followup.send(
            f"Could not load the script summary: {e}", ephemeral=True
        )


@script_commands.command(name="list", description="List all tracked scripts")
async def script_list(interaction: discord.Interaction):
    await interaction.response.defer()
    try:
        names = await asyncio.to_thread(unique_script_names)
        if not names:
            await interaction.followup.send(
                f"No scripts are tracked yet. [Open the Google Sheet]({SHEET_URL})"
            )
            return
        ...
        embed = discord.Embed(
            title=f"Tracked Scripts ({len(names)})",
            url=SHEET_URL,
            description=description,
            color=discord.Color.gold(),
        )
        embed.set_footer(text="Use /script summary name:<script> to see its work log")
        await interaction.followup.send(embed=embed)
    except Exception as e:
        print(f"/script list error: {e}", flush=True)
        await interaction.followup.send(
            f"Could not load the script list: {e}", ephemeral=True
        )


bot.tree.add_command(script_commands)
